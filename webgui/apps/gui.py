...
def smart_homing(motor, node_id, homing_current, homing_pos, expected_pos, homing_vel=10.0, homing_acc=10.0, homing_stall_time=0.2):
    try:
        ...
        tx_id = motor.build_can_id(dir_bit=0, cmd_id=17) 
        ...
        motor.bus.send(can.Message(arbitration_id=tx_id, data=struct.pack("<If", 23, float(homing_vel)), is_extended_id=False))
        time.sleep(0.05)
        motor.bus.send(can.Message(arbitration_id=tx_id, data=struct.pack("<If", 24, float(homing_acc)), is_extended_id=False))
        time.sleep(0.05)
        motor.bus.send(can.Message(arbitration_id=tx_id, data=struct.pack("<If", 25, float(homing_acc)), is_extended_id=False))
        time.sleep(0.05)
        ...
        if node_id == 7:
            message_queue.put(f"[{node_id}] 🛠️ 夹爪专属优化：探零前先往闭合方向微调，拉开加速空间...")
            motor.set_position(motor.position + 45.0)
            time.sleep(1.5)
        ...
        stall_threshold = max(1, int(homing_stall_time / 0.1))
        ...
        while True:
            ...
            if not has_backed_off and (elapsed > 2.0 or motor.status.over_current):
                if abs(pos - start_pos) < 2.0:
                    message_queue.put(f"[{node_id}] ⚠️ 发现起步即卡死，执行动态双向防卡死测试...")
                    motor.status.over_current = False
                    motor.disable()
                    time.sleep(0.1)
                    motor.error_resets()
                    ...
                    if node_id == 7:
                        ...
                        first_offset = 30.0 if homing_pos < 0 else -30.0

                    message_queue.put(f"[{node_id}] 🔄 正在尝试向 {first_offset}° 方向解锁回退...")
                    motor.set_position(pos + first_offset)
                    time.sleep(2.0)
                    
                    check_pos = motor.position
                    if abs(check_pos - pos) < 2.0:
                        second_offset = -first_offset
                        message_queue.put(f"[{node_id}] ⚠️ 该方向依然卡死！切换反方向 {second_offset}° 继续尝试解锁...")
                        motor.set_position(pos + second_offset)
                        time.sleep(2.0)
            ...
            if elapsed > 0.5:
                ...
                if stall_counter >= stall_threshold:
                    message_queue.put(f"[{node_id}] ✅ 成功检测到物理限位停转 (持续 {homing_stall_time} 秒高电流憋停)！")
                    break
...
def homing_all_motors(homing_vel=10.0, homing_acc=10.0, homing_stall_time=0.2):
    if homing_event.is_set():
    ...
    def _homing_task():
        homing_event.set()
        try:
            # 修改点：更新了每个元组的第三个参数 (expected_pos)
            homing_params = {
                1: (3.0, -360, 180.0),
                2: (4.0, -360, 80.0),
                3: (4.0, 360, -100.0),
            ...
            if motor5:
                ...
                if params5:
                    success, msg = smart_homing(motor5, 5, params5[0], params5[1], 0.0, homing_vel, homing_acc, homing_stall_time)
                    if msg == "BACKOFF_FAIL":
                        time.sleep(2)
                        success, msg = smart_homing(motor5, 5, params5[0], params5[1], 0.0, homing_vel, homing_acc, homing_stall_time)
                    if not success:
            ...
            for i in range(1, 8):
                ...
                if params:
                    # ---------------- 修改点：为夹爪(第七关节)单独降低速度 ----------------
                    if i == 7:
                        joint_vel = 5.0  # 夹爪速度减慢
                        joint_acc = 5.0
                    else:
                        joint_vel = homing_vel
                        joint_acc = homing_acc
                    # ----------------------------------------------------------------------

                    success, msg = smart_homing(motor, i, params[0], params[1], params[2], joint_vel, joint_acc, homing_stall_time)
                    if msg == "BACKOFF_FAIL":
...
with ui.tab_panels(tabs, value=menu1).classes('w-full'):
    ...
    with ui.tab_panel(menu1):
        with ui.card().classes('w-full'):
            ...
            ui.separator().classes('flex-grow')  
            ui.label('ARM Calibration (自动调零与归位)').classes('text-sm text-gray-500') 

            with ui.row().classes('w-full items-center mb-2'):
                ui.label('调零运动参数:').classes('text-sm text-gray-700 mr-2')
                auto_home_vel = ui.number('Homing Vel', value=10.0, format='%.1f').classes('w-24').tooltip("探零默认速度为 10.0")
                auto_home_acc = ui.number('Homing Acc', value=10.0, format='%.1f').classes('w-24').tooltip("探零默认加速度为 10.0")
                auto_home_stall = ui.number('Stall Time (s)', value=0.2, format='%.1f').classes('w-24').tooltip("憋停检测时间，默认 0.2 秒")

            with ui.row().classes('w-full items-center'):
                ui.button('HOMING', on_click=manual_homing_all_motors, color='green')
                ui.button('AUTO HOMING', on_click=lambda: homing_all_motors(auto_home_vel.value, auto_home_acc.value, auto_home_stall.value), color='green')
            ...
            with ui.row().classes('w-full items-center'):
                ...
                ui.button('Open Gripper', on_click=lambda: controller.motors[7].set_position(2.0) if 7 in controller.motors else None, color='teal')
                ui.button('Close Gripper', on_click=lambda: controller.motors[7].set_position(118.0) if 7 in controller.motors else None, color='teal')
