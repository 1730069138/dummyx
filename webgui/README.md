...
普通定位、整组姿态、循环和回放增加软件运行监控：目标误差超过 5°且连续 3 秒
没有至少 0.2°的有效位移时，向仍有新鲜位置反馈的相关关节发送当前位置保持目标，
随后取消任务并进入软件锁定。该功能不会自动失能，也不能代替安全 PLC、抱闸或硬件急停。
状态表会在距离软限位 5°以内预警，报警日志同时记录保护停止、软限位预警和 CAN 错误帧变化。
