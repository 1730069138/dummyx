...
def apply_virtual_wall_protection(model, data, ctrl_cmd, tcp_id, current_table_force, step_counter):
    ...
    # 🚨 时序门控与空间隔离：
    # 只要超过第 80 步（即提拉阶段），或者高度超过 0.26，立刻休眠，彻底解除安全干预！
    if step_counter > 80 or tcp_pos[2] > 0.26:
        global_virtual_wall_locked = False
        return ctrl_cmd, is_contact

    current_q = data.qpos[:6].copy()
    J_full = get_full_jacobian(model, data, tcp_id)
    
    q_dot_cmd = ctrl_cmd[:6] - current_q
    v_cart_cmd = J_full @ q_dot_cmd
    v_z_cmd = v_cart_cmd[2]  
    ...
    if global_virtual_wall_locked:
        ...
        if force_error > 0:
            # 🚨 修改 1：极其微弱的导纳系数（把弹簧变成海绵）
            K_admittance = 0.001  
            v_compensation = force_error * K_admittance
            
            # 🚨 修改 2：最高补偿速度压低至 1.5cm/s (0.015)，肉眼彻底看不出弹飞
            v_compensation = min(v_compensation, 0.015)
            v_cart_cmd[2] += v_compensation
        
        q_dot_safe = damped_pinv(J_full, 0.05) @ v_cart_cmd
        ctrl_cmd[:6] = current_q + q_dot_safe
