...
def run_perception_probe(
    ...
    rows = []
    summary_lines = [
        "VLSA/AEGIS V2.7 pure perception probe",
    ...
    print("\n" + "=" * 90)
    print("V2.7 PERCEPTION PROBE: no OpenPI connection, no robot motion")
...
def run_candidate_pair_probe(
    ...
    print("\n" + "=" * 96)
    print("V2.7 CROSS-VIEW CANDIDATE PAIR PROBE: no OpenPI connection, no robot motion")
    ...
    # Oracle-free ordering: smallest 3D center disagreement first, then shape
    # disagreement, then higher DINO confidence. No oracle term is used here.
    pair_rows.sort(
        key=lambda row: (
            row["center_distance_m"],
            row["extent_log_mismatch"],
            -row["confidence_mean"],
        )
    )
    ...
    summary_lines = [
        "VLSA/AEGIS V2.7 cross-view candidate pair probe",
        f"query={query}",
        "Ranking key: center_distance_m ASC, extent_log_mismatch ASC, confidence_mean DESC.",
...
def main(args):
    ...
    print("\n" + "=" * 90)
    print("VLSA/AEGIS V2.7 — all-candidate perception probe + full-action CBF-QP")
    print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "VLSA/AEGIS V2.7: all-candidate perception probe + MVEE + full-action CBF-QP + pickup-gate diagnosis "
