    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for idx, cfg in enumerate(configs):
            ...
            for episode in range(args.num_episodes):
                ...
                # 🚨 核心改动：计算并累加当前 episode 的总碰撞冲量 (步长约为0.1秒)
                # 使用 np.sum(f_z) * dt 可以完美计算出受力曲线下方的面积
                ep_impulse = np.sum(episode_data["f_z"]) * 0.1
                total_impulse += ep_impulse
                ...
                print(f"  └─ 回合 {episode+1}/{args.num_episodes}: {'✅ 成功' if is_success else '❌ 失败'} | {'💥 撞倒' if episode_col else '🛡️ 绕开'} | 最大力矩: {ep_max_t:.2f}N·m | 冲量: {ep_impulse:.2f}N·s")

            sr = (success_count / args.num_episodes) * 100
            cr = (collision_count / args.num_episodes) * 100
            avg_tau = total_max_tau / args.num_episodes
            avg_imp = total_impulse / args.num_episodes
            ...
            report_summary.append({
                ...
                "imp_str": f"{avg_imp:.2f} N·s"
            })

    # 🚨 终极修改：构造总结表格字符串，输出并保存至 txt 文件 (增加冲量列)
    summary_str = "\n================== 📜 最终消融实验总结表 (8工况) ==================\n"
    summary_str += "| 编号 | 障碍物状态 | 宏观避障策略 | 末端接触保护 | 任务成功率(SR) | 碰撞率(CR) | 平均最大力矩 | 平均碰撞冲量 |\n"
    summary_str += "| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |\n"
    for idx, res in enumerate(report_summary):
        summary_str += f"| Case {idx+1} | {res['obs_str']} | {res['apf_str']} | {res['cont_str']} | {res['sr_str']} | {res['cr_str']} | {res['tau_str']} | {res['imp_str']} |\n"
    summary_str += "========================================================================\n"

    # 在终端打印
    print(summary_str)

    # 导出到 txt 文件，方便论文引用和记录
