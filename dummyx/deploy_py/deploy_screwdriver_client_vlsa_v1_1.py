...
- Reproduces the full-action AEGIS control core (translation + rotation):
    end-effector ellipsoid + obstacle ellipsoid + auxiliary sphere state + CBF-QP.
- Adapts the user's absolute joint-target VLA output to a 6D end-effector twist,
  applies the AEGIS safety correction, then maps only the safety correction back
  to joint space.
...
# AEGIS full-action implementation settings.
CBF_ALPHA = 10.0
Z_ASCENT_GAIN = 10.0
QP_W_V = 1.0 / 25.0
QP_W_Z = 1.0
...
def compute_cbf_coeffs_world_twist(
    ...
    """
    Return AEGIS CBF coefficients for WORLD-frame translational and angular velocity.

    The official full-action implementation returns coefficients for body/local-frame
    velocity variables by right-multiplying the world coefficients by R_i. Here the
    nominal twist comes from MuJoCo Jacobians directly in the world frame, so we use
    eta_row and zeta_tilde themselves.

    Returns:
        a_v_world:     coefficient on world linear velocity, shape (3,)
        a_omega_world: coefficient on world angular velocity, shape (3,)
        a_u_z:         coefficient on auxiliary control u_z, shape (3,)
        h:             current CBF value
        mu_row:        dh/dz
    """
    q_i = np.diag(q_i_diag)
    ...
    # Orientation derivative from the official compute_h_coeffs_3d implementation.
...
def damped_pinv(jacobian, rho=DLS_RHO):
    """Damped least-squares right pseudoinverse."""
...
class AEGISFullActionLayer:
    """Full-action VLSA/AEGIS CBF-QP for one EEF/obstacle ellipsoid pair."""
    ...
    def filter(self, eef_center, eef_rotation, v_nominal_world, omega_nominal_world):
        ...
        # Official full-action AEGIS: [v(3), omega(3), u_z(3)].
        u = cp.Variable(9)
        weight = np.diag(
            [QP_W_V, QP_W_V, QP_W_V, QP_W_V, QP_W_V, QP_W_V,
...
def adapt_joint_target_through_vlsa(
    ...
    """
    Convert the user's absolute joint target to a nominal 6D EEF twist, apply
    full-action AEGIS, then map only the safety correction back to joint space.

    V1 used only translation while still allowing the nominal joint target to rotate
    the EEF ellipsoid. That omitted orientation term can violate the CBF model. This
    version includes the rotational derivative exactly as the official full-action
    AEGIS implementation does.
    """
    q = data.qpos[:6].copy()
    ...
    jacp_full, jacr_full = get_point_jacobians(
        model, data, eef_center, link7_id
    )
    j_pos = jacp_full[:, :6]
    j_rot = jacr_full[:, :6]
    ...
    v_safe, omega_safe, debug = safety_layer.filter(
        eef_center, eef_rotation, v_nom, omega_nom
    )
...
def main(args):
    ...
    print("\n" + "=" * 84)
    print("VLSA/AEGIS V1.1 — full-action CBF-QP control-core reproduction")
    ...
    if args.debug_gate_after_pickup:
        print("DEBUG ONLY: CBF is gated off until screwdriver z > 0.235 m.")
    print("=" * 84 + "\n")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for episode in range(args.num_episodes):
            ...
            if args.mode == "vlsa" and use_obstacle:
                ...
                safety_layer = AEGISFullActionLayer(p_obs, r_obs, q_obs, action_dt)
            ...
            while viewer.is_running() and step_counter < args.max_steps:
                ...
                use_cbf_this_step = safety_layer is not None
                if args.debug_gate_after_pickup and screw_z_before <= 0.235:
                    # Diagnostic only: NOT the official VLSA baseline.
                    # If this makes the task succeed, the failure is specifically
                    # caused by the distance safety layer interfering with reach/grasp.
                    use_cbf_this_step = False

                if use_cbf_this_step:
            ...
            max_screw_z = float(np.max(ep_screw_z)) if ep_screw_z else float(data.xpos[target_body_id][2])
            picked_up = max_screw_z > 0.235
            qp_text = f" | picked={'YES' if picked_up else 'NO'} | z_obj_max={max_screw_z:.3f}"
            if ep_qp_times:
                ...
                qp_text += (
                    ...
                    f" | intervention_mean={np.mean(ep_interventions):.4f}"
                    f" | intervention_max={np.max(ep_interventions):.4f}"
                )
            print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="VLSA/AEGIS V1.1 full-action control-core reproduction for screwdriver pick-and-place"
    )
    ...
    parser.add_argument(
        ...
        default="vlsa",
        help="baseline: original VLA adapter; vlsa: AEGIS full-action CBF-QP",
    )
    ...
    parser.add_argument(
        "--debug_gate_after_pickup",
        action="store_true",
        help=(
            "Diagnostic only: disable VLSA before screwdriver z exceeds 0.235 m. "
            "Do not use this flag for the final VLSA baseline."
        ),
    )
