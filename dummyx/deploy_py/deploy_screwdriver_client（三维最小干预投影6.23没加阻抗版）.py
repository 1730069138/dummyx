...
def get_target_table_force(model, data, valid_body_ids):
    ...
    return f_xyz
# ==========================================
# 🆕 基于 TCP 高度的无状态虚拟墙防护 (高稳抓取平滑版)
# ==========================================
global_last_v_comp = np.zeros(3)

def apply_virtual_wall_protection(model, data, ctrl_cmd, tcp_id, current_f_xyz, step_counter):
    global global_last_v_comp
    ...
    if tcp_pos[2] < 0.235:
        ...
        if force_error > 0:
            ...
            # 🚨 核心防甩魔法：强行压制 XY 平面的横向退让，强迫其主要向 Z 轴垂向逃逸
            f_dir[0] *= 0.1  # X轴响应降为 10%
            f_dir[1] *= 0.1  # Y轴响应降为 10%
            f_dir = f_dir / (np.linalg.norm(f_dir) + 1e-6) # 重新归一化
            
            v_ideal = force_error * K_admittance * f_dir
            
            # 🚨 限速器压低：最高逃逸速度限制在极其平滑的 0.05m/s (5厘米/秒)
            v_norm = np.linalg.norm(v_ideal)
            if v_norm > 0.05:
                v_ideal = (v_ideal / v_norm) * 0.05
            v_compensation = v_ideal
            
        # 3. 气囊滤波：最高 alpha=0.1，保持极佳的记忆棉平滑度，绝不触电
        alpha = np.clip(0.02 + 0.01 * force_norm, 0.02, 0.1)
        global_last_v_comp = alpha * v_compensation + (1 - alpha) * global_last_v_comp

    else:
        # 🚨 核心防掉魔法：“松油门”平滑衰减，绝不急刹车！
        # 飞出安全区后，残余的补偿速度以 92% 的比例极速且顺滑地衰减
        global_last_v_comp = 0.92 * global_last_v_comp
    ...
    # 绝对物理防穿透二次保护 (防止残余补偿计算出错)
    if is_wall_active and tcp_pos[2] < 0.212 and v_target_3d[2] < 0:
        v_target_3d[2] = 0.0
...
def main(args):
    ...
    if 1 <= args.case <= 8:
        ...
        configs = all_configs
        print(f"\n🚀 [全量消融模式] 将按顺序测试全部 8 种工况，每种工况 {args.num_episodes} 次。")
    ...
    fixed_targets = None
    if args.fixed_eval:
        print("🔒 [固定评测模式] 开启！当前工况将使用完全相同的螺丝刀初始位置。")
        np.random.seed(42) 
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for cfg in configs:
            ...
            run_folder_name = f"run_{timestamp}_Case{cfg['case_id']}_{cfg['tag_obs']}_{cfg['tag_apf']}_{cfg['tag_cont']}"
            base_record_dir = os.path.join(BASE_DIR, "recordings", run_folder_name)
            os.makedirs(base_record_dir, exist_ok=True)

            print("\n" + "="*80)
            print(f" 📊 [运行工况 Case {cfg['case_id']}] 障碍: [{cfg['str_obs']}] | APF: [{cfg['str_apf']}] | 保护: [{cfg['str_cont']}]")
            print("="*80)
            ...
            for episode in range(args.num_episodes):
                    ...
                    for _ in range(50):
                        ...
                        if cfg["contact"]:
                            current_ctrl, wall_active_step = apply_virtual_wall_protection(
                                model, data, base_ctrl.copy(), tcp_id, current_f_xyz, step_counter
                            )
                            if wall_active_step: 
                                is_contact_ui = True
                        else:
                ...
                np.savez(data_filename, 
                         ...
                         f_real_x=np.array(episode_data["f_real_x"]), # <- 新写入
                         f_real_y=np.array(episode_data["f_real_y"]), # <- 新写入
                         f_real_z=np.array(episode_data["f_real_z"]), # <- 新写入
    ...
    summary_str = "\n========================================= 📜 最终消融实验总结表 =========================================\n"
    summary_str += "| 编号 | 障碍物状态 | 宏观避障策略 | 末端接触保护 | 任务成功率(SR) | 碰撞率(CR) | 平均最大力矩 | 瞬时冲力峰值 | 冲量(全量) | 成功冲量(均值±标准差) | 成功冲量方差 |\n"
    summary_str += "| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |\n"
    for res in report_summary:
        summary_str += f"| Case {res['case_id']} | {res['obs_str']} | {res['apf_str']} | {res['cont_str']} | {res['sr_str']} | {res['cr_str']} | {res['tau_str']} | {res['peak_f_str']} | {res['imp_all_str']} | {res['imp_succ_str']} | {res['imp_var_str']} |\n"
    summary_str += "=========================================================================================================\n"

    print(summary_str)

    txt_path = os.path.join(BASE_DIR, "recordings", f"ablation_summary_{timestamp}.txt")
