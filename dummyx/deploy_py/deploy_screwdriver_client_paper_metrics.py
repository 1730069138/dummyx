...
def main(args):
    ...
    # ==========================================
    paper_groups = [
        {"group_id": "B1",  "obs": False, "apf": False, "contact": False, "mode": "none",       "obs_str": "✗", "avoid_str": "✗", "contact_str": "—"},
        {"group_id": "B2",  "obs": False, "apf": True,  "contact": False, "mode": "none",       "obs_str": "✗", "avoid_str": "✓", "contact_str": "—"},
        {"group_id": "B3",  "obs": False, "apf": False, "contact": True,  "mode": "admittance", "obs_str": "✗", "avoid_str": "✗", "contact_str": "I"},
        {"group_id": "B4",  "obs": False, "apf": False, "contact": True,  "mode": "impedance",  "obs_str": "✗", "avoid_str": "✗", "contact_str": "II"},
        {"group_id": "B5",  "obs": False, "apf": True,  "contact": True,  "mode": "admittance", "obs_str": "✗", "avoid_str": "✓", "contact_str": "I"},
        {"group_id": "B6",  "obs": False, "apf": True,  "contact": True,  "mode": "impedance",  "obs_str": "✗", "avoid_str": "✓", "contact_str": "II"},
        {"group_id": "B7",  "obs": True,  "apf": False, "contact": False, "mode": "none",       "obs_str": "✓", "avoid_str": "✗", "contact_str": "—"},
        {"group_id": "B8",  "obs": True,  "apf": True,  "contact": False, "mode": "none",       "obs_str": "✓", "avoid_str": "✓", "contact_str": "—"},
        {"group_id": "B9",  "obs": True,  "apf": False, "contact": True,  "mode": "admittance", "obs_str": "✓", "avoid_str": "✗", "contact_str": "I"},
        {"group_id": "B10", "obs": True,  "apf": False, "contact": True,  "mode": "impedance",  "obs_str": "✓", "avoid_str": "✗", "contact_str": "II"},
        {"group_id": "B11", "obs": True,  "apf": True,  "contact": True,  "mode": "admittance", "obs_str": "✓", "avoid_str": "✓", "contact_str": "I"},
        {"group_id": "B12", "obs": True,  "apf": True,  "contact": True,  "mode": "impedance",  "obs_str": "✓", "avoid_str": "✓", "contact_str": "II"},
    ]

    if 1 <= args.group <= 12:
        test_queue = [paper_groups[args.group - 1]]
        print(f"\n🎯 [论文消融单组模式] 仅运行 B{args.group}，共 {args.num_episodes} 回合。")
    else:
        test_queue = paper_groups
        print("\n🚀 [论文完整消融模式] 按 B1-B12 顺序运行全部 12 组。")

    print("========================================================")
    print(f"⚙️  测试队列已构建：共 {len(test_queue)} 组")
    print("========================================================\n")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for cfg in test_queue:
            ...
            episode_peak_forces = []       # 全部回合峰值，仅用于保留原始统计
            succ_peak_forces = []          # f_hat：仅成功回合的每回合峰值接触力
            all_impulses = []              # I：全部回合接触冲量
            succ_impulses = []             # I_succ / sigma_I：仅成功回合接触冲量
            
            run_folder_name = f"run_{timestamp}_{cfg['group_id']}_{cfg['mode']}_obs{int(cfg['obs'])}_avoid{int(cfg['apf'])}_contact{int(cfg['contact'])}"
            base_record_dir = os.path.join(BASE_DIR, "recordings", run_folder_name)
            os.makedirs(base_record_dir, exist_ok=True)

            print("\n" + "="*80)
            print(f" 📊 [运行组 {cfg['group_id']}] 障碍物: [{cfg['obs_str']}] | 几何避障: [{cfg['avoid_str']}] | 接触调节: [{cfg['contact_str']}]")
            print("="*80)
            ...
            for episode in range(args.num_episodes):
                ...
                if is_success:
                    succ_peak_forces.append(ep_true_peak_f)
                ...
                np.savez(data_filename, 
                         ...
                         f_norm_peak=np.array(episode_data["f_z"]),
                         f_z=np.array(episode_data["f_z"]),  # 兼容旧分析脚本；实际为每动作步接触力模长峰值
                ...
                print(f"  └─ 回合 {episode+1}/{args.num_episodes}: {'✅ 成功' if is_success else '❌ 失败'} | {'💥 撞倒' if episode_col else '🛡️ 绕开'} | 峰值关节扭矩: {ep_max_t:.2f} N·m | 峰值接触力: {ep_true_peak_f:.2f} N | 接触冲量: {ep_true_impulse:.2f} N·s")

            sr = (success_count / args.num_episodes) * 100
            # 无障碍组 CR 不适用，汇总时显示“—”
            cr = (collision_count / args.num_episodes) * 100 if cfg["obs"] else None
            # tau_bar_max：各回合峰值关节扭矩的平均值
            avg_tau = total_max_tau / args.num_episodes

            # f_hat：仅成功回合；先取每个成功回合的峰值接触力，再跨成功回合求均值
            avg_peak_f_succ = np.mean(succ_peak_forces) if succ_peak_forces else None
            # I：全部回合接触冲量均值
            avg_imp_all = np.mean(all_impulses) if all_impulses else 0.0
            # I_succ：成功回合接触冲量均值
            avg_imp_succ = np.mean(succ_impulses) if succ_impulses else None
            # sigma_I：成功回合接触冲量标准差（用户确认口径 B）
            std_imp_succ = np.std(succ_impulses) if succ_impulses else None

            report_summary.append({
                "group_id": cfg["group_id"],
                "obs_str": cfg["obs_str"],
                "avoid_str": cfg["avoid_str"],
                "contact_str": cfg["contact_str"],
                "sr_str": f"{sr:.1f}%",
                "cr_str": f"{cr:.1f}%" if cr is not None else "—",
                "peak_f_str": f"{avg_peak_f_succ:.2f} N" if avg_peak_f_succ is not None else "—",
                "imp_all_str": f"{avg_imp_all:.2f} N·s",
                "imp_succ_str": f"{avg_imp_succ:.2f} N·s" if avg_imp_succ is not None else "—",
                "sigma_i_str": f"{std_imp_succ:.2f} N·s" if std_imp_succ is not None else "—",
                "tau_str": f"{avg_tau:.2f} N·m"
            })

    summary_str = "\n============================== 📜 PACE B1-B12 消融实验汇总 ==============================\n"
    summary_str += "| 组号 | 障碍物 | 几何避障 | 接触调节 | SR (%) | CR (%) | f_hat (N) | I (N·s) | I_succ (N·s) | sigma_I (N·s) | tau_bar_max (N·m) |\n"
    summary_str += "| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |\n"
    for res in report_summary:
        summary_str += (
            f"| {res['group_id']} | {res['obs_str']} | {res['avoid_str']} | {res['contact_str']} | "
            f"{res['sr_str']} | {res['cr_str']} | {res['peak_f_str']} | {res['imp_all_str']} | "
            f"{res['imp_succ_str']} | {res['sigma_i_str']} | {res['tau_str']} |\n"
        )
    summary_str += "========================================================================================\n"

    print(summary_str)

    txt_path = os.path.join(BASE_DIR, "recordings", f"pace_ablation_B1-B12_{timestamp}.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write(summary_str)
    print(f"📝 总结报告已同步保存至: {txt_path}\n")

if __name__ == "__main__":
    p = argparse.ArgumentParser(description="PACE 论文 B1-B12 消融实验部署脚本")
    ...
    p.add_argument("--num_episodes", default=100, type=int, help="每个 B 组测试回合数（默认：100）")
    p.add_argument(
        "--group", "--case",
        dest="group",
        default=0,
        type=int,
        help="指定论文消融组 B1-B12；0 表示按 B1-B12 顺序运行全部 12 组。--case 为兼容旧命令的别名。"
    )
    p.add_argument("--fixed_eval", action="store_true", help="启用固定初始化评测模式。")
    p.add_argument(
        "--control_mode",
        type=str,
        default="both",
        choices=["admittance", "impedance", "both"],
        help=argparse.SUPPRESS
    )

    args = p.parse_args()
    if args.control_mode != "both":
        print("⚠️ --control_mode 在论文 B1-B12 模式下不再筛选测试组；形式 I/II 已由 B 组编号固定。")
    main(args)