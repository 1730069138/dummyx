...
def main(args):
    ...
    print("\n" + "=" * 90)
    print("VLSA/AEGIS V2.1 — perception MVEE + full-action CBF-QP")
    print(
        ...
        for episode in range(args.num_episodes):
            ...
            while viewer.is_running() and step_counter < args.max_steps:
                ...
                if safety_layer is not None:
                    ...
                    # MuJoCo body pose, including tilt after contact/knockdown.
                    # This is used only for oracle debugging; perception mode
                    # never reads MuJoCo ground-truth pose for control.
                    if args.obstacle_source == "oracle":
            ...
            if ep_qp_times:
                ...
                qp_text += (
                    ...
                    f" | intervention_mean="
                    f"{np.mean(ep_interventions):.4f}"
                    f" | intervention_max="
                    f"{np.max(ep_interventions):.4f}"
                )
            else:
                qp_text += (
                    f" | safety=OFF"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "VLSA/AEGIS V2.1: perception MVEE + full-action CBF-QP "
